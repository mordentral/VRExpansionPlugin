...
void UGS_GunTools::AddRecoilInstance(const FTransform & RecoilAddition, FVector Optional_Location)
{
	...
	else
	{
		...
		curRot.Pitch = FMath::Clamp(curRot.Pitch, FMath::Min(-MaxRecoilRotation.Y, MaxRecoilRotation.Y), FMath::Max(MaxRecoilRotation.Y, 0.f));
		curRot.Yaw = FMath::Clamp(curRot.Yaw, FMath::Min(-MaxRecoilRotation.Z, MaxRecoilRotation.Z), FMath::Max(MaxRecoilRotation.Z, 0.f));
		curRot.Roll = FMath::Clamp(curRot.Roll, FMath::Min(-MaxRecoilRotation.X, MaxRecoilRotation.X), FMath::Max(MaxRecoilRotation.X, 0.f));
