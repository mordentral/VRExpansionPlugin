...
void UGS_GunTools::AddRecoilInstance(const FTransform & RecoilAddition, FVector Optional_Location)
{
	...
	else
	{
		...
		CurVec.X = FMath::Clamp(CurVec.X, -MaxRecoilTranslation.X, MaxRecoilTranslation.X);
		CurVec.Y = FMath::Clamp(CurVec.Y, -MaxRecoilTranslation.Y, MaxRecoilTranslation.Y);
		CurVec.Z = FMath::Clamp(CurVec.Z, -MaxRecoilTranslation.Z, MaxRecoilTranslation.Z);
		BackEndRecoilTarget.SetTranslation(CurVec);

		FVector CurScale = BackEndRecoilTarget.GetScale3D();
		CurScale.X = FMath::Clamp(CurScale.X, -MaxRecoilScale.X, MaxRecoilScale.X);
		CurScale.Y = FMath::Clamp(CurScale.Y, -MaxRecoilScale.Y, MaxRecoilScale.Y);
		CurScale.Z = FMath::Clamp(CurScale.Z, -MaxRecoilScale.Z, MaxRecoilScale.Z);
		BackEndRecoilTarget.SetScale3D(CurScale);

		FRotator curRot = BackEndRecoilTarget.Rotator();
		curRot.Pitch = FMath::Clamp(curRot.Pitch, -MaxRecoilRotation.Y, MaxRecoilRotation.Y);
		curRot.Yaw = FMath::Clamp(curRot.Yaw, -MaxRecoilRotation.Z, MaxRecoilRotation.Z);
		curRot.Roll = FMath::Clamp(curRot.Roll, -MaxRecoilRotation.X, MaxRecoilRotation.X);
