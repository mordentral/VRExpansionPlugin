...
FReply FVRGlobalSettingsDetails::OnCorrectInvalidAnimationAssets()
{
	...
	for (auto& Asset : AssetList)
	{
		...
		{
			{		
				float FrameRateD = AnimSeq->GetFrameRate();
				...
				if (FrameRateD < 1.0f)
				{
					...
					AnimSeq->SequenceLength = 1.f;
					AnimSeq->SetRawNumberOfFrame(1);
	...
	UE_LOG(LogAnimation, Log, TEXT("Finished fixing animation assets!"));
